...
def _decorate_home(page) -> None:
    # Mantém a lógica original, mas aproxima cartões e botões da tela de referência.
...
def _decorate_news(page) -> None:
    for button_name in ("custom", "stop"):
        _secondary(getattr(page, button_name, None))
...
def _decorate_extractor(page) -> None:
    # Central de downloads: botão amarelo, cartões e ações secundárias.
...
def _decorate_pdf(page) -> None:
    # A página já possui toda a lógica do editor. O refresh visual é feito sem trocar
    # sinais/slots nem alterar o motor PDF.
...
def apply_reference_layout(window) -> None:
    ...
    # Tabelas gerais: aparência leve das referências.
