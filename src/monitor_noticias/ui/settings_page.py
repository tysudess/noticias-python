...
class SettingsPage(BasePage):
    def __init__(self, controller: MainUiController) -> None:
        ...
        self.proxy_hint = QLabel("Informe servidor, usuário e senha para autenticação.")
        self.proxy_hint.setObjectName("settingsMuted")
        titles.addWidget(title)
        titles.addWidget(self.proxy_hint)
        head.addLayout(titles, 1)

        enabled_text = QLabel("Ativado")
        enabled_text.setObjectName("settingsStrong")
        head.addWidget(enabled_text)
        ...
        fields = QHBoxLayout()
        fields.setSpacing(8)
        self.host = self._field(fields, "Servidor", QLineEdit())
        self.port = self._field(fields, "Porta", QSpinBox())
        self.port.setRange(1, 65535)
        self.user = self._field(fields, "Usuário", QLineEdit())
        self.password = self._field(fields, "Senha", QLineEdit())
        ...
        self.news_interval.addItems(["15", "30", "45", "60", "120"])

        self.dem_auto = QCheckBox()
        self.dem_interval = QComboBox()
        self.dem_interval.addItems(["15", "30", "45", "60", "120"])

        self.video_auto = QCheckBox()

        services.addWidget(self._service_card(
            "▤", "Notícias", "Varredura das fontes selecionadas",
            self.news_auto, self.news_interval, "blue"
        ), 1)
        services.addWidget(self._service_card(
            "▣", "Demandas", "Pesquisa de todas as demandas ativas",
            self.dem_auto, self.dem_interval, "orange"
        ), 1)
        services.addWidget(self._service_card(
            "▶", "Vídeos", "Execução nos horários definidos abaixo",
            self.video_auto, None, "purple"
        ), 1)
        ...
        hint = QLabel(
            "Use HH:MM separados por vírgula. Ex: 08:00, 12:00, 15:00, 19:00, 21:00"
        )
    ...
    @staticmethod
    def _toggle_row(title_text, subtitle_text, checkbox):
        frame = QFrame()
    ...
    def _service_card(self, icon_text, title_text, subtitle_text, checkbox, combo, tone):
        frame = QFrame()
    ...
    def _stylesheet(self) -> str:
        ...
        QLabel#settingsMuted { color:#6079A5;font-size:10px; }
        QLabel#settingsSaved {
            background:#EAF9F2;color:#078B5F;border:1px solid #C2EBD8;
            border-radius:9px;padding:10px 14px;font-size:10px;font-weight:800;
        }
        QLabel#settingsBlueIcon {
            background:#E5F1FF;color:#087AF7;border-radius:11px;
            font-size:22px;font-weight:900;
        }
        QLabel#settingsTitle { color:#08245F;font-size:17px;font-weight:900; }
        QLabel#settingsStrong { color:#08245F;font-size:11px;font-weight:900; }
        QLabel#settingsFieldLabel { color:#244B7B;font-size:10px; }
        QPushButton#settingsPrimary {
            background:#0A7DF8;color:white;border:0;border-radius:8px;
            padding:9px 15px;font-weight:800;
        }
        ...
        QLabel#settingsSuccess {
            ...
            border-radius:7px;padding:8px 10px;font-size:10px;
        }
        QFrame#settingsToggleRow { border-bottom:1px solid #DFEAF6; }
        ...
        QLabel#settingsServiceIcon[tone='blue'] { background:#E5F1FF;color:#087AF7; }
        QLabel#settingsServiceIcon[tone='orange'] { background:#FFF0CC;color:#EA9900; }
        QLabel#settingsServiceIcon[tone='purple'] { background:#F0E4FF;color:#8244F5; }
        """
    ...
    def _proxy_done(self, ok, text):
        self.proxy_message.setText(("✓  " if ok else "⚠  ") + text)
    ...
    def _apply_auto(self):
        ...
        s.news_automatic = self.news_auto.isChecked()
        s.demand_automatic = self.dem_auto.isChecked()
        s.video_automatic = self.video_auto.isChecked()
        s.news_interval_minutes = int(self.news_interval.currentText())
        s.demand_interval_minutes = int(self.dem_interval.currentText())
    ...
    def refresh(self, _state: UiState) -> None:
        ...
        self.proxy_hint.setText(
            ...
            if cfg.host else "Informe os dados do servidor proxy."
        )

        s = self.controller.automation_settings
        self.general.setChecked(s.automatic_monitoring)
        self.news_auto.setChecked(s.news_automatic)
        self.dem_auto.setChecked(s.demand_automatic)
        self.video_auto.setChecked(s.video_automatic)
        self.news_interval.setCurrentText(str(s.news_interval_minutes))
        self.dem_interval.setCurrentText(str(s.demand_interval_minutes))
